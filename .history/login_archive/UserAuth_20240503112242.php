...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { // POST METHOD
    ...
    if (isset($_POST['submit'])) { // INPUT SUBMIT
        ...
        try{
            ...
            $sql = $pdo->prepare("SELECT i_id_entidade, s_name_empresa, s_email_entidade, s_password_entidade FROM sistema WHERE s_email_entidade = :s_email_entidadeSELECT i_id_entidade, empresa.s_name_empresa, s_email_entidade, s_password_entidade FROM entidade INNER JOIN empresa WHERE empresa.i_id_empresa = entidade.fk_enterprise_entidade AND s_email_entidade = :s_email_entidade"); // Prepara o statement para escapar os dados para execução
