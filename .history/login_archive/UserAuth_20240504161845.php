...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { // POST METHOD
    ...
    if (isset($_POST['submit'])) { // INPUT SUBMIT
        ...
        try{
            ...
            $sql = $pdo->prepare("SELECT i_id_entidade, fk_permissons_entidade, empresa.s_name_empresa, s_email_entidade, s_password_entidade FROM entidade WHERE s_email_entidade = :s_email_entidadeSELECT i_id_entidade, empresa.s_name_empresa, s_email_entidade, s_password_entidade FROM entidade INNER JOIN empresa WHERE empresa.i_id_empresa = entidade.fk_enterprise_entidade AND s_email_entidade = :s_email_entidade"); // Prepara o statement para escapar os dados para execução
            ...
            if ($user) { // VERIFICA O USUÁRIO
                ...
                if ($password == $user['s_password_entidade']) { // VERIFICA SENHA // ps: password_verify() só verifica senhas previamente criadas por password_hash()
                        ...
                        $_SESSION['Name'] = $user['s_name_empresa'];
